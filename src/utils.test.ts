...
Deno.test("should b64ToUtf8", () => {
  ...
  assertEquals(encoded, "SGFp");
  const decoded = b64ToUtf8("SGFp");
  assertEquals(decoded, "Hai");
});
...
Deno.test("should truncateUrl", () => {
  ...
  assertEquals(
    truncateUrl(url, 50),
    "https://www.example.com/a/long/path?q=important",
  );
  assertEquals(
    truncateUrl(url, 46),
    "https://example.com/a/long/path?q=important",
  );
